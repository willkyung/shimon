...
let toastTimer = null;
let adminDeletionInProgress = false;
...
const initialAuthLinkType = initialAuthHash.get('type') || initialAuthQuery.get('type') || '';
let passwordRecoveryMode = initialAuthLinkType === 'recovery';
let emailConfirmationMode = ['signup', 'email'].includes(initialAuthLinkType);
...
remoteDatabase?.auth.onAuthStateChange((event) => {
    if (event === 'PASSWORD_RECOVERY') {
        ...
        setTimeout(showAdminPasswordUpdate, 0);
    }

    if (event === 'SIGNED_IN' && emailConfirmationMode) {
        setTimeout(() => showAuthView('email-confirmed'), 0);
    }
});
...
function showAuthView(viewName) {
    const map = {
        ...
        'recovery-account': 'authRecoveryAccount',
        'recovery-password': 'authRecoveryPassword',
        'email-confirmed': 'authEmailConfirmed'
    };
...
async function handleAdminSignup(event) {
    ...
    if (!remoteDatabase) {
        showToast('Supabase 연결 정보를 확인해주세요.');
        return;
    }
    ...
    setButtonBusy(submitButton, true, '계정 생성 중...');
    ...
    try {
        ...
        if (data.session) await remoteDatabase.auth.signOut();

        verifiedAdmin = null;
        showAuthView('login');
        setValue('loginIdentifier', account.email);
        setValue('signupPassword', '');
        setValue('signupPasswordConfirm', '');
        showToast(data.session
            ? '관리자 회원가입이 완료되었습니다. 로그인해주세요.'
        ...
        if (message === 'already_registered' || /already registered|already exists|duplicate/i.test(message)) {
            showToast('이미 사용 중인 이메일 또는 사원코드입니다.');
        }
        else if (code === 'over_email_send_rate_limit' || error?.status === 429) {
            showToast('메일 요청이 너무 많습니다. 잠시 후 다시 시도해주세요.');
        }
        else if (/Database error saving new user/i.test(message)) {
            showToast('이미 사용 중인 이메일 또는 사원코드인지 확인해주세요.');
        }
        else {
            showToast(`회원가입 실패: ${message || '잠시 후 다시 시도해주세요.'}`);
        }
    }
    finally {
...
function fillAdminDemoLogin() {
    setValue('loginIdentifier', 'admin@shimon.com');
...
async function handleAdminLogin(event) {
    ...
    try {
        ...
        currentAdmin = await getRemoteCurrentAdmin();
        if (!currentAdmin || currentAdmin.role !== 'admin') {
            await remoteDatabase.auth.signOut();
            currentAdmin = null;
            showToast('관리자 계정만 로그인할 수 있습니다.');
            return;
        }
        ...
        if (error?.code === 'email_not_confirmed') {
            ...
            showToast('관리자 프로필을 찾지 못했습니다. SQL 설정을 확인해주세요.');
        }
        else {
...
async function logoutAdmin(showMessage = true) {
    if (remoteDatabase) await remoteDatabase.auth.signOut();
    currentAdmin = null;
...
async function getRemoteCurrentAdmin() {
    if (!remoteDatabase) return null;

    const { data: sessionData, error: sessionError } = await remoteDatabase.auth.getSession();
    ...
    if (error) throw error;
    if (!profile) {
        const missing = new Error('profile_not_found');
        missing.code = 'profile_not_found';
        throw missing;
    }

    return profileToAdmin(profile);
}
...
async function handleAdminAccountRecovery(event) {
    event.preventDefault();
    const button = $('#accountRecoveryButton');
    const result = $('#accountRecoveryResult');

    if (!remoteDatabase || !result) {
        showToast('Supabase 연결 정보를 확인해주세요.');
        return;
    }
    ...
    try {
        const { data, error } = await remoteDatabase.rpc('find_shimon_admin_account', {
            p_employee_code: normalizeEmployeeCode($('#recoveryEmployeeCode')?.value),
            p_name: $('#recoveryName')?.value.trim() || '',
            p_phone: $('#recoveryPhone')?.value.trim() || ''
        });
        if (error) throw error;
        ...
        const isMissingFunction =
            error?.code === 'PGRST202' ||
            /find_shimon_admin_account|schema cache|could not find the function/i.test(
                String(error?.message || '')
            );

        if (isMissingFunction) {
            result.textContent =
                'Supabase SQL Editor에서 admin-account-recovery.sql을 한 번 실행해주세요.';
            result.classList.remove('hidden');
            showToast('관리자 이메일 찾기 DB 설정이 아직 적용되지 않았습니다.');
        }
        else {
            showToast('이메일 찾기에 실패했습니다. 잠시 후 다시 시도해주세요.');
        }
    }
    finally {
...
async function handleAdminPasswordRecoveryRequest(event) {
    event.preventDefault();
    ...
    if (!/^https?:$/.test(location.protocol)) {
        showToast('Live Server로 실행한 뒤 다시 시도해주세요.');
        return;
    }

    const button = $('#passwordRecoveryButton');
    setButtonBusy(button, true, '메일 전송 중...');

    try {
        const redirectTo = `${location.origin}${location.pathname}`;
        const { error } = await remoteDatabase.auth.resetPasswordForEmail(
        ...
        console.error('재설정 메일 전송 실패:', error);
        showToast(error?.status === 429
            ? '메일 요청이 너무 많습니다. 잠시 후 다시 시도해주세요.'
            : '재설정 메일을 보내지 못했습니다.');
    }
    finally {
        setButtonBusy(button, false);
...
function showAdminPasswordUpdate() {
    $('#authShell')?.classList.remove('hidden');
    $('#adminApp')?.classList.add('hidden');
    showAuthView('recovery-password');
...
async function handleAdminPasswordUpdate(event) {
    ...
    const button = $('#passwordUpdateButton');
    ...
    try {
        ...
        showToast('비밀번호를 변경하지 못했습니다. 재설정 링크를 다시 요청해주세요.');
    }
    finally {
        setButtonBusy(button, false);
    }
...
async function finishAdminEmailConfirmation() {
...
function openAdminDeleteAccountModal() {
    $('#adminDeleteAccountModal')?.classList.remove('hidden');
}

function closeAdminDeleteAccountModal() {
    if (!adminDeletionInProgress) {
        $('#adminDeleteAccountModal')?.classList.add('hidden');
    }
}

function handleAdminDeleteBackdrop(event) {
    if (event.target?.id === 'adminDeleteAccountModal') {
        closeAdminDeleteAccountModal();
    }
}

async function deleteAdminAccount() {
    if (!remoteDatabase || adminDeletionInProgress) return;

    adminDeletionInProgress = true;
    const button = $('#confirmAdminDeleteButton');
    setButtonBusy(button, true, '탈퇴 처리 중...');
    ...
    try {
        ...
        showToast('관리자 계정이 탈퇴 처리되었습니다.');
    }
    catch (error) {
        console.error('계정 탈퇴 실패:', error);
        showToast('계정 탈퇴에 실패했습니다. SQL 설정을 확인해주세요.');
    }
    finally {
        adminDeletionInProgress = false;
        setButtonBusy(button, false);
    }
}
...
window.addEventListener(
    ...
    async () => {
        ...
        if (loginIdentifier) {
            ...
            loginIdentifier.placeholder = 'admin@company.com';
            loginIdentifier.autocomplete = 'email';
            loginIdentifier.closest('label')?.querySelector('span')?.replaceChildren('이메일');
        }
        $('#authLogin .auth-demo')?.classList.add('hidden');

        try {
            currentAdmin = await getRemoteCurrentAdmin();
        }
        catch (error) {
            console.error('관리자 세션 복원 실패:', error);
            currentAdmin = null;
        }

        if (passwordRecoveryMode) {
            showAdminPasswordUpdate();
            return;
        }

        if (emailConfirmationMode) {
            $('#authShell')?.classList.remove('hidden');
            $('#adminApp')?.classList.add('hidden');
            showAuthView('email-confirmed');
            return;
        }
        ...
        if (
            ...
            if (currentAdmin && remoteDatabase) {
                await remoteDatabase.auth.signOut();
            }
            currentAdmin = null;
